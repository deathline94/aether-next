...
fn read_anchor() -> (PathBuf, Vec<u8>, Vec<(String, String)>) {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap_or_else(|e| {
            die(
                &format!("{} is missing or unresolvable: {e}", ANCHOR_REL),
...
fn main() {
    ...
    code.push_str(&format!(
        "// @generated by build.rs from {} — do not edit, and do not regenerate from the artifacts.\n",
        ANCHOR_REL
    ));
    code.push_str(
    ...
    code.push_str(&format!(
        "/// Verbatim bytes of the anchor this binary was built against, so the running shell can \
         ...
         pub static ENGINE_TRUST_ANCHOR_BYTES: &[u8] = include_bytes!({:?});\n",
        anchor_path
    ));
    code.push_str(&format!(
