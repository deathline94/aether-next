...
use aether_desktop_lib::{
    file_sha256_hex, verify_elevated_binary, BinaryTrustError, TrustedBinaryPolicy,
};
use std::fs;
...
fn test_elevation_trust_rejects_hash_mismatch() {
    ...
    let hashes: &'static [(&'static str, &'static str)] =
        Box::leak(vec![(filename, wrong_hash)].into_boxed_slice());
    let policy = TrustedBinaryPolicy {
        #[cfg(debug_assertions)]
        allow_unsigned_for_dev: true,
        expected_publisher_cn: "deathline94",
    ...
    match res {
        Err(BinaryTrustError::HashMismatch {
            expected, actual, ..
        }) => {
...
fn embedded_hashes_are_sourced_from_the_reviewed_anchor_not_from_the_artifact() {
    ...
    let files = doc["files"]
        .as_array()
        .expect("anchor needs a files[] array");
    ...
    for entry in files {
        ...
        let found = aether_desktop_lib::EMBEDDED_RELEASE_HASHES
            ...
            .unwrap_or_else(|| {
                panic!(
                    "{name} has an anchor entry but no embedded digest — an absent \
...
fn anchor_digests_match_the_committed_artifacts_they_describe() {
    let dll =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../packaging/wintun.dll");
...
fn test_elevation_trust_rejects_missing_hash_when_enforced() {
    ...
    assert!(
        res.is_err(),
        "Missing hash must be rejected when enforce_hash_match=true"
    );
    match res {
...
fn trusted_binary_roots_are_the_named_layout_directories_only() {
    ...
    assert!(roots
        .iter()
        .any(|r| r == &app.join("resources").canonicalize().unwrap()));
    assert!(roots
        .iter()
        .any(|r| r == &app.join("engine").canonicalize().unwrap()));
    assert!(
    ...
    let resolved = sibling
        .canonicalize()
        .unwrap_or_else(|_| base.join("OtherVendor").join("aether.exe"));
    assert!(
