import { Radio, Search, X, Zap } from "lucide-react";
      ...
      <section className="settings-section">
        ...
        <div className="setting-row input-row">
          <label>
            ...
            <NumberField label="Scan concurrency" min={1} max={2000} value={concurrency} disabled={active} onCommit={setConcurrency} />
          </label>
          <label>
            <span>Per-Probe Timeout (ms)</span>
            <NumberField label="Per-probe timeout in milliseconds" min={100} max={30000} value={timeoutMs} disabled={active} onCommit={setTimeoutMs} />
          </label>
        </div>
          ...
          <div className="scan-inline-progress">
            ...
            >
              <div
                ...
                style={{ width: scanState.total > 0 ? `${Math.min(100, Math.round((scanState.scanned / scanState.total) * 100))}%` : "0%" }}
              />
            </div>
        ...
        <div className="scanner-action-bar">
          {!active ? (
            <button type="button" className="primary-cta connect" onClick={startScan} disabled={busy}>
              <Zap size={18} aria-hidden="true" />
              <span>{busy ? "Starting…" : "Start Standalone Scan"}</span>
            </button>
          ) : (
            <button type="button" className="primary-cta disconnect" onClick={stopScan}>
      ...
      <section className="discovered-panel">
        ...
        {endpoints.length === 0 ? (
          <div className="empty-logs">
            ...
            <span>Tap "Start Standalone Scan" to probe healthy edge IPs.</span>
          </div>
        ) : (
            ...
            {endpoints.map((item) => (
              <div className="discovered-row" key={item.addr}>
                ...
                <div className="discovered-actions">
                  <span className={`rtt-badge ${item.rttMs <= 50 ? "fast" : item.rttMs <= 120 ? "mid" : "slow"}`}>{item.rtt}</span>
                  <button
                    type="button"
