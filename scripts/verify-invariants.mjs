 ...
 * obfuscation::RECOGNIZED_PROFILES is the authority every shell has to agree with.

 * Each drift found in this vocabulary during the audit came from somebody keeping a

 * second copy of the list - the Android table did, and it cost the phone a rung of

 * its noise ladder - so a gate that checks the shells reads the original instead of
...
function engineNoiseVocabulary(api) {
  ...
  const file = api
    ...
    .files('aether/src', new RegExp('rs' + '$'))

    .find((x) => rel(x).endsWith('/obfuscation.rs'));

  if (!file) return null;

  const text = api.read(file).replace(new RegExp('[' + String.fromCharCode(13) + ']', 'g'), '');

  const at = text.indexOf('RECOGNIZED_PROFILES');

  if (at < 0) return null;

  // The first bracket after the name is in the type annotation (`&[&str]`), so

  // anchor on the first quoted entry and take the brackets around it.

  const quote = text.indexOf(String.fromCharCode(34), at);

  if (quote < 0) return null;

  const open = text.lastIndexOf(String.fromCharCode(91), quote);

  const close = text.indexOf(String.fromCharCode(93), quote);

  if (open < 0 || close < quote) return null;
...
const GATES = [
...

  {
    ...
    scan(api) {
      ...
      const BASELINE = new Map([
        ...
        // 52 -> 51: desktop gained hydration-side port correction (out-of-range
        // ports fall back to the default and are reported); Android deliberately
        // refuses the whole payload as corrupt instead (see the coerce-vs-refuse
        // note in either types.ts), so the fix has no twin to sync here.
        ['types.ts', 51],
  ...
  },
  {
    ...
    scan(api) {
      ...
      if (!noise) {
        return [
          'aether/src/obfuscation.rs: RECOGNIZED_PROFILES could not be read, so the noise field would have gone unchecked rather than reported',
        ];
      }
      const accepted = new Set(
